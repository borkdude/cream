...
6. `JAVA_HOME` is needed for classes Crema loads at runtime from the JDK's
   `lib/modules`. Clojure code and the tested libraries do not hit this. When
   `java.home` points at a tree without `lib/modules`, `BootClassRegistry`
   prints a warning to stdout and falls back.
